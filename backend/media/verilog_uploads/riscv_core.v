...
module riscv_core(input wire clk, input wire reset);
    ...
    // ALU
    alu ALU(.in1(rs1_data), .in2(alu_in2), .alu_op(alu_op), .out(alu_result));
...
// ================== IF STAGE ==================
module if_stage(input wire clk, input wire reset, output wire [31:0] pc_out, output wire [31:0] instr_out);
    reg [31:0] pc;
    wire [31:0] instr;
...
module id_stage(
    ...
    always @(*) begin
        ...
        rs2 = instr[24:20];
        rd  = instr[11:7];
        imm_out = {{20{instr[31]}}, instr[31:20]};  // default I-type immediate
